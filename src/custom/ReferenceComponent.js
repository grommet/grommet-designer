...
export default ({ design, selected, value, onChange }) => {
  const [components, setComponents] =
    React.useState(Object.keys(design.components)
      .filter(id => isValid(design.components[id])));
  return (
    <Box>
      <FormField label="component">
        <Select
          ...
          onSearch={(text) => {
            ...
            setComponents(Object.keys(design.components).filter(id => {
